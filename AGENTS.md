# AGENTS.md — redpen / Scott

This file makes **redpen** work in any agent that reads `AGENTS.md` (Claude Code,
Cursor, and friends), without the plugin installed. Drop it in a repo root, or
paste it into your agent's rules.
...
When the user is opening/updating/finalizing a pull request, or says "review my
diff", "check before I push", "red-pen this", or "is this ready to ship",
perform a redpen review.
...
3. A diff/range the user points you at.

Only flag **added or changed** lines (the `+` side). Leave untouched code alone.

## The catch-list

- **Debug output** — `console.log`, `print`, `debugger`, `dd()`, `var_dump`,
  leftover breakpoints, watch-the-value logs.
...
- **Placeholder & junk text** — `lorem ipsum`, `asdf`, `test123`, `foo`/`bar`
  as real values, "remove me", profanity.
- **Hardcoded local/test values** — `localhost`/`127.0.0.1`, personal paths
  (`/Users/<name>/…`), personal names/emails, magic test data, dev-only ports.
- **Stray secrets** — API keys, tokens, passwords, connection strings, private
  keys. Flag first and loudest; never echo a secret in full — mask it.
- **Test artifacts** — `.only`/`.skip`/`fit`/`fdescribe`, skipped assertions,
  `xit`, `test.todo`.
...
one plain sentence on why it's a leftover. End with a one-line verdict.

If the diff is clean, say so briefly and stop — don't invent problems.
